...
public class Mathstuff {
	...
	public void calcR3(double[][][] coords, double[] forward, double[] camPos,double alpha, double beta,double factor) {	//f:forward vector; a:position of camera, alpha:rotation x2, beta:rotation x3
		this.updateValues();
...
//		double fovFactor = 0.5 * (1/Math.tan(Math.toRadians(fov/2)));
	...
	public double calcR3Depth(double[] point, double[] cameraPos) {
		calcR3DepthVectorCache = new double[] {point[0]-cameraPos[0],point[1]-cameraPos[1],point[2]-cameraPos[2]};
		return Math.sqrt((calcR3DepthVectorCache[0]*calcR3DepthVectorCache[0])+(calcR3DepthVectorCache[1]*calcR3DepthVectorCache[1])+(calcR3DepthVectorCache[2]*calcR3DepthVectorCache[2]));
	}
	...
	/**
	 * this is a cache of length two, which holds the most recent calculated point coordinates(onscreen pixels) calculated in calcR3Point
	 */
//	private int[] coordsINTCache;
	/**
	 * returns depth of point if rotated back to the camera's original forward direction
	 ...
	 * <br><br>
	 * needs to be private, because its values need to be updated in every frame. This should be done in other methods from this class invoking this method 
	 */
//	private double calcR3Point(double[] coords, double[] forward, double[] camPos,double alpha, double beta, double factor){
//		
//		//System.out.println("Gegeben: X1"+coords[0]+",X2"+coords[1]+",X3"+coords[2]+",forwardX1"+forward[0]+",camPosX1"+camPos[0]);
//		cacheAnkerEbene = new double[] {forward[0] + camPos[0],forward[1] + camPos[1],forward[2] + camPos[2]};			//z:"angriffspunkt ebene"
//		
//		cacheVectorCamToPoint0 = new double[] {coords[0]-camPos[0],coords[1]-camPos[1],coords[2]-camPos[2]}; //b:vector camera to point
...
//			vecCamToEbenenSchnittpunkt = new double[] {lambdaCamToPointEbenenSchnittpunkt * cacheVectorCamToPoint0[0],lambdaCamToPointEbenenSchnittpunkt * cacheVectorCamToPoint0[1],lambdaCamToPointEbenenSchnittpunkt * cacheVectorCamToPoint0[2]};	//jetzt:vektor kamera->schnittpunkt
//			//rotate that back around x3 axis
//			vecCamToEbenenSchnittpunktX3 = new double[] {(Math.cos(-beta)*vecCamToEbenenSchnittpunkt[0] - Math.sin(-beta)*vecCamToEbenenSchnittpunkt[1]), Math.sin(-beta)*vecCamToEbenenSchnittpunkt[0] + Math.cos(-beta)*vecCamToEbenenSchnittpunkt[1], (vecCamToEbenenSchnittpunkt[2])};
//			//rotate that back around x2 axis			
//			coordsINTCache = new int[] {0, screenCenterX + (int)((vecCamToEbenenSchnittpunktX3[1]*factor)*screenSizeMinimum*fovFactor), screenCenterY-(int)(((-Math.sin(-alpha)*vecCamToEbenenSchnittpunktX3[0] + Math.cos(-alpha)*vecCamToEbenenSchnittpunktX3[2])*factor)*screenSizeMinimum*fovFactor)};
//		    
...
//	}
	 ...
	 * needs to be private, because its values need to be updated in every frame. This should be done in other methods from this class invoking this method 
	 */
	private double calcR3Point(double[] coords, int[] coordsIntCache, double[] forward, double[] camPos,double alpha, double beta, double factor){
...
//			vecCamToEbenenSchnittpunkt = new double[] {lambdaCamToPointEbenenSchnittpunkt * cacheVectorCamToPoint0[0],lambdaCamToPointEbenenSchnittpunkt * cacheVectorCamToPoint0[1],lambdaCamToPointEbenenSchnittpunkt * cacheVectorCamToPoint0[2]};	//jetzt:vektor kamera->schnittpunkt
			...
			//rotate that back around x3 axis
			vecCamToEbenenSchnittpunktX3[0] = (Math.cos(-beta)*vecCamToEbenenSchnittpunkt[0] - Math.sin(-beta)*vecCamToEbenenSchnittpunkt[1]);
			vecCamToEbenenSchnittpunktX3[1] = Math.sin(-beta)*vecCamToEbenenSchnittpunkt[0] + Math.cos(-beta)*vecCamToEbenenSchnittpunkt[1];
			vecCamToEbenenSchnittpunktX3[2] = vecCamToEbenenSchnittpunkt[2];
			
//			vecCamToEbenenSchnittpunktX3 = new double[] {(Math.cos(-beta)*vecCamToEbenenSchnittpunkt[0] - Math.sin(-beta)*vecCamToEbenenSchnittpunkt[1]), Math.sin(-beta)*vecCamToEbenenSchnittpunkt[0] + Math.cos(-beta)*vecCamToEbenenSchnittpunkt[1], (vecCamToEbenenSchnittpunkt[2])};
			//rotate that back around x2 axis
			coordsIntCache[0] = screenCenterX + (int)(vecCamToEbenenSchnittpunktX3[1]*factor*screenSizeMinimum*fovFactor);
			coordsIntCache[1] = screenCenterY-(int)(((-Math.sin(-alpha)*vecCamToEbenenSchnittpunktX3[0] + Math.cos(-alpha)*vecCamToEbenenSchnittpunktX3[2])*factor)*screenSizeMinimum*fovFactor); 
//			coordsINTCache = new int[] {0, screenCenterX + (int)((vecCamToEbenenSchnittpunktX3[1]*factor)*screenSizeMinimum*fovFactor), screenCenterY-(int)(((-Math.sin(-alpha)*vecCamToEbenenSchnittpunktX3[0] + Math.cos(-alpha)*vecCamToEbenenSchnittpunktX3[2])*factor)*screenSizeMinimum*fovFactor)};
...
//			System.out.println("t " + triangleI);
			...
			//Vektor AC
			ac = new double[] {coords[triangleI][2][0]-coords[triangleI][0][0],coords[triangleI][2][1]-coords[triangleI][0][1],coords[triangleI][2][2]-coords[triangleI][0][2]};
			double acLength = length(ac);
			ac = new double[] {ac[0]/acLength,ac[1]/acLength,ac[2]/acLength};
			
			//Vektor BC
			bc = new double[]{coords[triangleI][2][0]-coords[triangleI][1][0],coords[triangleI][2][1]-coords[triangleI][1][1],coords[triangleI][2][2]-coords[triangleI][1][2]};
//			bcLength = length(bc);
...
//			precision = 1;
			...
			//lambdaAB gibt die Stelle auf der Geraden AB*lambda + A an, bei der ab senkrecht zu o (o = Stelle auf der Gerade -> C)			
			lambdaAB =
...
//				System.out.println("---");
			...
			//Vektor O (Einheitsvektor) (steht senkrecht auf AB und geht durch (bzw. bis) C)
			o  = new double[] {coords[triangleI][2][0]-(coords[triangleI][0][0]+ab0[0]*lambdaAB),coords[triangleI][2][1]-(coords[triangleI][0][1]+ab0[1]*lambdaAB),coords[triangleI][2][2]-(coords[triangleI][0][2]+ab0[2]*lambdaAB)};
			...
			for(double lambdaABCrawler = 0; lambdaABCrawler < abLength + precision; lambdaABCrawler += precision) {				
				...
				//check if lambdaVertical should be calculated by collision with BC
				...
				//distinguish between the two different calculation methods for lambda2Max, depending on if its a collision with AC or CB				
				if(collidingWithAC) {
...
//							(o[0]*ac[1]-o[1]*ac[0]);
					...
					//TODO NOTE: this should be cleaned up, as triangles are now reprocessed in the main loop 
					//1.: calculate divisor and check if it is 0. If so, calculate it with two other points. If it is again 0, set lambdaVertical to 0
					...
					if(cacheLambda2Divisor == 0) {
						...
						if(cacheLambda2Divisor == 0) {
							...
							if(cacheLambda2Divisor == 0) {
//								System.err.println("WARNING: This model contains an invalid triangle (at calculating R3zBUFF) " + triangleI + ", " + lambdaABCrawler);
//								System.out.println("---");
//								System.out.println(Arrays.toString(coords[triangleI][0]));
//								System.out.println(Arrays.toString(coords[triangleI][1]));
//								System.out.println(Arrays.toString(coords[triangleI][2]));
//								System.out.println("---");
								break; //this tringle is weird (e.g. just a line) and can't be rendered
							}else {
								lambda2Max = (lambdaABCrawler*ab0[2]*ac[1]-lambdaABCrawler*ab0[1]*ac[2])   /   cacheLambda2Divisor;
							}
						}else {
							lambda2Max = (lambdaABCrawler*ab0[2]*ac[0]-lambdaABCrawler*ab0[0]*ac[2])   /   cacheLambda2Divisor;
						}
					} else {
						lambda2Max = (lambdaABCrawler*ab0[1]*ac[0]-lambdaABCrawler*ab0[0]*ac[1])   /   cacheLambda2Divisor;						
					}
...
//							if(cacheLambda2Divisor == 0) {
//								System.out.println("WARNING: This model contains an invalid triangle (at calculating R3zBUFF) " + triangleI + ", " + lambdaABCrawler);
//								System.out.println("---");
//								System.out.println(Arrays.toString(coords[triangleI][0]));
//								System.out.println(Arrays.toString(coords[triangleI][1]));
//								System.out.println(Arrays.toString(coords[triangleI][2]));
...
//								break; //this tringle is weird (e.g. just a line) and can't be rendered
//							}else {
//								lambda2Max = (lambdaABCrawler*ab0[2]*bc[1]-lambdaABCrawler*ab0[1]*bc[2]-ab[2]*bc[1]+ab[1]*bc[2])   /   cacheLambda2Divisor;
//							}
						}else {
							lambda2Max = (lambdaABCrawler*ab0[0]*bc[2]-lambdaABCrawler*ab0[2]*bc[0]-ab[0]*bc[2]+ab[2]*bc[0])   /   cacheLambda2Divisor;
						}
					} else {
						lambda2Max = (lambdaABCrawler*ab0[0]*bc[1]-lambdaABCrawler*ab0[1]*bc[0]-ab[0]*bc[1]+ab[1]*bc[0])   /   cacheLambda2Divisor;						
					}
...
//						System.err.println("WHELP " + lambda2Max);
				...
				for(double lambda2 = 0; lambda2 < lambda2Max + precision; lambda2 += precision) {
					//calculate the point (=> 2D into coordsIntCache) and return its depth
					depth = calcR3Point(
							new double[] {lambda2*o[0] + coords[triangleI][0][0] + lambdaABCrawler*ab0[0], lambda2*o[1] + coords[triangleI][0][1] + lambdaABCrawler*ab0[1],lambda2*o[2] + coords[triangleI][0][2] + lambdaABCrawler*ab0[2]},
...
//						depth = calcR3Point(new double[] {lambda2Max*o[0] + coords[triangleI][0][0] + lambdaABCrawler*ab0[0],lambda2Max*o[1] + coords[triangleI][0][1] + lambdaABCrawler*ab0[1],lambda2Max*o[2] + coords[triangleI][0][2] + lambdaABCrawler*ab0[2]},forward,camPos,alpha,beta,factor);
//						//System.out.println("Bekommen X:"+(int)R3PointEnd[1]+" ;Y:"+(int)R3PointEnd[2]+" ;Deep:"+(int)R3PointEnd[3]+" - Final");
//						if(coordsINTCache[1]>0&&coordsINTCache[2]>0&&coordsINTCache[1]<screenWidth&&coordsINTCache[2]<screenHeight&&(Buffer2D[coordsINTCache[1]][coordsINTCache[2]] > depth||Buffer2D[coordsINTCache[1]][coordsINTCache[2]]==0))
//						{
...
//					}
					...
					//TODO is the following if complex still needed? => bc the coordinates are already manipulated to prevent (lambdaABCrawler+precision > abLength) from happening 
					//this ensures the last pixel (at lambaAB, lambda2 = 0) is drawn. not needed bc. for loop makes sure of that
...
//							break;
//						}
//						for(double lambda2 = 0;lambda2<=lambda2Max;lambda2+=precision)
//						{
//							depth = calcR3Point(new double[] {lambda2*o[0] + coords[triangleI][0][0] + abLength*ab0[0],lambda2*o[1] + coords[triangleI][0][1] + abLength*ab0[1],lambda2*o[2] + coords[triangleI][0][2] + abLength*ab0[2]},forward,camPos,alpha,beta,factor);
//							if(coordsINTCache[1]>0&&coordsINTCache[2]>0&&coordsINTCache[1]<screenWidth&&coordsINTCache[2]<screenHeight&&(Buffer2D[coordsINTCache[1]][coordsINTCache[2]] > depth||Buffer2D[coordsINTCache[1]][coordsINTCache[2]]==0))
//							{
//								Buffer2D[coordsINTCache[1]][coordsINTCache[2]] = depth;
//								//System.out.println(Arrays.toString(coordsINTCache)+", length: "+lengthB);
...
//								depth = calcR3Point(new double[] {lambda2Max*o[0] + coords[triangleI][0][0] + abLength*ab0[0],lambda2Max*o[1] + coords[triangleI][0][1] + abLength*ab0[1],lambda2Max*o[2] + coords[triangleI][0][2] + abLength*ab0[2]},forward,camPos,alpha,beta,factor);
...
//					for(double lambda2 = 0; lambda2 < lambda2Max + precision; lambda2 += precision) {
////						System.out.println("hmh " + lambda2 + ", lambdaMax: " + lambda2Max);
//						//Gefundener Punkt
...
//						depth = calcR3Point(new double[] {lambda2*o[0] + coords[triangleI][0][0] + lambdaABCrawler*ab0[0],lambda2*o[1] + coords[triangleI][0][1] + lambdaABCrawler*ab0[1],lambda2*o[2] + coords[triangleI][0][2] + lambdaABCrawler*ab0[2]},forward,camPos,alpha,beta,factor);
...
//					}
	...
	private void printWarningValuesNotInitialized() {
		System.err.println("-----WARNING----- Values of Mathstuff Object were not initialized at this point. INFO: Thread: '" + Thread.currentThread().toString() +  "'");		
	}
}
